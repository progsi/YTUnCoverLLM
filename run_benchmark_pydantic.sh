    ...
    for k in "${array[@]}"; do
        ...
        if [ "$k" -eq 0 ]; then
            ...
            for sampling in rand tfidf; do
                python run_ie_pydantic.py --llm $model -i "data/dataset/"$dataset"/dataset"$d_id"/test.IOB" -k $k -s $sampling -o "output/"$dataset"/"$model"/dataset"$d_id"_"$k"shot_"$sampling".jsonl" 
            done
        fi  
    done
done
